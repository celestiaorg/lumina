...
impl ShareApi {
    ...
    ///
    /// The response is returned as received from the node and is not verified,
    /// use [`ShareApi::get_samples_verified`] to verify it against a header you trust.
    ...
    /// Retrieves multiple shares from the [`ExtendedDataSquare`] at the given
    /// sample coordinates and verifies each of them against the given header.
    ///
    /// `coordinates` is a list of `(row, column)`.
    ...
    ///
    /// The response is returned as received from the node and is not verified,
    /// use [`ShareApi::get_eds_verified`] to verify it against a header you trust.
    ...
    /// Retrieves the complete [`ExtendedDataSquare`] and verifies it against the
    /// given header.
    pub async fn get_eds_verified(&self, header: &ExtendedHeader) -> Result<ExtendedDataSquare> {
    ...
    ///
    /// The response is returned as received from the node and is not verified,
    /// use [`ShareApi::get_row_verified`] to verify it against a header you trust.
    ...
    /// Retrieves all shares from a specific row of the [`ExtendedDataSquare`]
    /// and verifies them against the given header.
    pub async fn get_row_verified(
        &self,
        header: &ExtendedHeader,
        row: u16,
    ) -> Result<GetRowResponse> {
    ...
    ///
    /// The response is returned as received from the node and is not verified,
    /// use [`ShareApi::get_namespace_data_verified`] to verify it against a header you trust.
    ...
    /// Retrieves all shares that belong to the specified namespace within the
    /// [`ExtendedDataSquare`] and verifies against the given header that they
    /// are all the shares of the namespace.
    ///
    /// The shares are returned in a row-by-row order, maintaining the original
    /// layout if the namespace spans multiple rows.
    ...
    ///
    /// The response is returned as received from the node and is not verified,
    /// use [`ShareApi::get_range_verified`] to verify it against a header you trust.
    ...
    /// Retrieves a list of shares and their corresponding proof, verifying against
    /// the given header that the proof covers exactly the requested range and that
    /// the shares are the proven ones.
    ///
    /// The start and end index ignores parity shares and corresponds to ODS.
