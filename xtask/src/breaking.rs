//! Diagnostic-only breaking-change detection for `prepare-release`: per crate,
//! decide whether a release is breaking and give a short reason (never influences
//! the version).
//!
//! Two signals; either alone marks a crate breaking:
//!
//! 1. API — `cargo-semver-checks` diffs the public API against the last published
//!    baseline (library crates only).
//! 2. Intent — author-declared breaking changes in the Conventional Commits (`!`
//!    marker / `BREAKING CHANGE:` footer).
//!
//! A missing or broken `cargo-semver-checks` fails open to the intent signal
//! (`ApiStatus::Unavailable`) rather than aborting.
...
///
/// Fail-open and total: never errors or panics on a tool problem — a missing or
/// broken `cargo-semver-checks` degrades to [`ApiStatus::Unavailable`] and the
/// intent signal alone decides `has_breaking`.
///
/// * `repo_root` — working directory for the `cargo-semver-checks` subprocess.
/// * `crate_info` — the crate under analysis (its `manifest_path` is diffed).
/// * `baseline` — the crate's last published version; `None` ⇒ never published.
/// * `parsed_commits` — parsed commits touching this crate since the baseline.
pub fn analyze(
    repo_root: &Path,
    ...
    let (api_status, api_reason) = if !has_library_target(&crate_info.manifest_path) {
        // Binary-only / cdylib (e.g. lumina-node-wasm) — no Rust library API to diff.
...
/// The intent signal: `Some(reason)` if any commit is breaking (`!` marker or
/// `BREAKING CHANGE:` footer, via [`commit::is_breaking`]), else `None`.
///
/// Precedence: the first breaking-footer text wins; otherwise the first
/// `!`-marked commit's reconstructed subject.
...
/// Reconstruct a Conventional-Commit subject `<kind>[(<scope>)]!: <description>`
/// from the parsed fields (`ParsedCommit` doesn't carry the raw subject).
...
/// Run `cargo-semver-checks` for one crate and interpret the result. A spawn
/// failure (tool not installed) fails open to [`ApiStatus::Unavailable`].
...
/// Interpret a finished `cargo-semver-checks` invocation (pure, unit-testable):
///
/// * exit 0 → `(Compatible, None)`.
/// * non-zero with a report → `(Incompatible, Some(ANSI-stripped text))`.
...
fn interpret_semver_checks(
    ...
    // Report goes to stderr; fall back to stdout. A non-empty report means real
    // incompatibilities.
...
///
/// `false` for binary-only crates and for crates whose library is exclusively a
/// `cdylib`/`staticlib` (e.g. `lumina-node-wasm`) — nothing to diff. Reads
/// `[lib].crate-type` from `Cargo.toml`; a read failure is treated conservatively
/// as "no library target".
